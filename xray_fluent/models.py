...
class AppSettings:
    ...
    # --- Стабильность / Health Monitor ---
    health_check_enabled: bool = True
    health_check_interval_sec: int = 15
    health_latency_warn_ms: int = 800
    health_latency_dead_ms: int = 3000
    health_dead_strikes: int = 3
    auto_ping_interval_sec: int = 120
    zapret_with_vpn: bool = True
    # "speed" = выбирать лучший по скорости, "ping" = по пингу, "roundrobin" = по очереди
    auto_switch_strategy: str = "speed"

    # ── Стабильность ──────────────────────────────────────────────────────────
    # Keepalive-пинг: переподключить если нет ответа подряд N раз (0 = выкл)
    stability_keepalive_enabled: bool = True
    stability_keepalive_interval_sec: int = 30
    stability_keepalive_fails_before_reconnect: int = 3
    # Экспоненциальный backoff при провале reconnect
    stability_backoff_enabled: bool = True

    # ── Анти-блокировки DPI ───────────────────────────────────────────────────
    # Фрагментация TLS ClientHello (обход DPI по первому пакету)
    dpi_fragment_enabled: bool = False
    dpi_fragment_packets: int = 2      # на сколько пакетов разбивать
    dpi_fragment_length: int = 50      # максимальная длина фрагмента (байт)
    dpi_fragment_interval_ms: int = 10 # интервал между фрагментами (мс)
    # Multiplexing: несколько логических потоков в одном TCP-соединении
    dpi_mux_enabled: bool = False
    dpi_mux_concurrency: int = 8
    # Xray-native шум (padding noise)
    dpi_noise_enabled: bool = False

    # --- Авто-загрузка конфигов (vpn-vless-configs-russia) ---
    config_fetch_enabled: bool = False          # включить периодическую загрузку
    config_fetch_interval_min: int = 60         # интервал в минутах
    config_fetch_workers: int = 20              # параллельных HTTP-потоков
    config_fetch_filter: bool = True            # фильтровать только РФ/Европа
    config_fetch_dedup: bool = True             # дедупликация по IP:port:scheme
    config_fetch_extra_urls: list = None        # дополнительные URL подписок

    def __post_init__(self) -> None:
        if self.config_fetch_extra_urls is None:
            object.__setattr__(self, "config_fetch_extra_urls", [])
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "health_latency_warn_ms": self.health_latency_warn_ms,
            "health_latency_dead_ms": self.health_latency_dead_ms,
            "health_dead_strikes": self.health_dead_strikes,
            "auto_ping_interval_sec": self.auto_ping_interval_sec,
            "zapret_with_vpn": self.zapret_with_vpn,
            "auto_switch_strategy": self.auto_switch_strategy,
            # stability
            "stability_keepalive_enabled": self.stability_keepalive_enabled,
            "stability_keepalive_interval_sec": self.stability_keepalive_interval_sec,
            "stability_keepalive_fails_before_reconnect": self.stability_keepalive_fails_before_reconnect,
            "stability_backoff_enabled": self.stability_backoff_enabled,
            # dpi
            "dpi_fragment_enabled": self.dpi_fragment_enabled,
            "dpi_fragment_packets": self.dpi_fragment_packets,
            "dpi_fragment_length": self.dpi_fragment_length,
            "dpi_fragment_interval_ms": self.dpi_fragment_interval_ms,
            "dpi_mux_enabled": self.dpi_mux_enabled,
            "dpi_mux_concurrency": self.dpi_mux_concurrency,
            "dpi_noise_enabled": self.dpi_noise_enabled,
            # config fetch
            "config_fetch_enabled": self.config_fetch_enabled,
            "config_fetch_interval_min": self.config_fetch_interval_min,
            "config_fetch_workers": self.config_fetch_workers,
            "config_fetch_filter": self.config_fetch_filter,
            "config_fetch_dedup": self.config_fetch_dedup,
            "config_fetch_extra_urls": list(self.config_fetch_extra_urls or []),
        }
    ...
    def from_dict(data: dict[str, Any]) -> "AppSettings":
        return AppSettings(
            ...
            health_check_enabled=bool(data.get("health_check_enabled", True)),
            health_check_interval_sec=int(data.get("health_check_interval_sec") or 15),
            health_latency_warn_ms=int(data.get("health_latency_warn_ms") or 800),
            health_latency_dead_ms=int(data.get("health_latency_dead_ms") or 3000),
            health_dead_strikes=int(data.get("health_dead_strikes") or 3),
            auto_ping_interval_sec=int(data.get("auto_ping_interval_sec") or 120),
            zapret_with_vpn=bool(data.get("zapret_with_vpn", True)),
            auto_switch_strategy=str(data.get("auto_switch_strategy") or "speed"),
            # stability
            stability_keepalive_enabled=bool(data.get("stability_keepalive_enabled", True)),
            stability_keepalive_interval_sec=int(data.get("stability_keepalive_interval_sec") or 30),
            stability_keepalive_fails_before_reconnect=int(data.get("stability_keepalive_fails_before_reconnect") or 3),
            stability_backoff_enabled=bool(data.get("stability_backoff_enabled", True)),
            # dpi
            dpi_fragment_enabled=bool(data.get("dpi_fragment_enabled", False)),
            dpi_fragment_packets=int(data.get("dpi_fragment_packets") or 2),
            dpi_fragment_length=int(data.get("dpi_fragment_length") or 50),
            dpi_fragment_interval_ms=int(data.get("dpi_fragment_interval_ms") or 10),
            dpi_mux_enabled=bool(data.get("dpi_mux_enabled", False)),
            dpi_mux_concurrency=int(data.get("dpi_mux_concurrency") or 8),
            dpi_noise_enabled=bool(data.get("dpi_noise_enabled", False)),
            config_fetch_enabled=bool(data.get("config_fetch_enabled", False)),
            config_fetch_interval_min=int(data.get("config_fetch_interval_min") or 60),
            config_fetch_workers=int(data.get("config_fetch_workers") or 20),
            config_fetch_filter=bool(data.get("config_fetch_filter", True)),
            config_fetch_dedup=bool(data.get("config_fetch_dedup", True)),
            config_fetch_extra_urls=list(data.get("config_fetch_extra_urls") or []),
        )
