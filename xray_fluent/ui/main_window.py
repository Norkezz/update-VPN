...
def _resource_path(filename: str) -> str:
    ...
    # Dev mode: file is in project root
...
from .config_fetch_page import ConfigFetchPage
...
class MainWindow(FluentWindow):
    def __init__(self, force_minimized: bool = False, defer_init: bool = False):
        ...
        self.setWindowIcon(QIcon(_resource_path("aegisnet_icon.ico")))
        self.resize(1280, 720)

        # Listen for bring-to-front requests from a second instance attempt
        self._setup_single_instance_listener()

        if not defer_init:
            self.initialize()

    def _setup_single_instance_listener(self) -> None:
        """Create a named Windows event; poll it to bring window to front."""
        if sys.platform != "win32":
            return
        import ctypes
        import sys as _sys
        kernel32 = ctypes.windll.kernel32  # type: ignore[attr-defined]
        event_name = "Global\\AegisNET_BringToFront"
        # Create (or open) a manual-reset event
        self._btf_event = kernel32.CreateEventW(None, False, False, event_name)
        if not self._btf_event:
            return
        from PyQt6.QtCore import QTimer
        self._btf_timer = QTimer(self)
        self._btf_timer.setInterval(500)
        self._btf_timer.timeout.connect(self._check_bring_to_front)
        self._btf_timer.start()

    def _check_bring_to_front(self) -> None:
        import ctypes
        kernel32 = ctypes.windll.kernel32  # type: ignore[attr-defined]
        _WAIT_OBJECT_0 = 0
        rc = kernel32.WaitForSingleObject(self._btf_event, 0)
    ...
    def initialize(self) -> None:
        ...
        self.config_fetch_page = ConfigFetchPage(self)
    ...
    def _create_navigation(self) -> None:
        ...
        self.addSubInterface(self.config_fetch_page, FIF.DOWNLOAD, "Конфиги")
    ...
    def _create_tray(self) -> None:
        ...
        self.tray.setIcon(QIcon(_resource_path("aegisnet_icon.ico")))
    ...
    def _connect_signals(self) -> None:
        ...
        self.dashboard_page.connect_fastest_requested.connect(self.controller.connect_selected)
        ...
        self.dashboard_page.zapret_start_requested.connect(self._on_zapret_start)
        self.dashboard_page.zapret_stop_requested.connect(self._on_zapret_stop)
        self.dashboard_page.auto_switch_toggled.connect(self._on_dashboard_auto_switch_toggled)
        self.dashboard_page.dpi_fragment_toggled.connect(self._on_dashboard_dpi_fragment_toggled)
        self.dashboard_page.dpi_mux_toggled.connect(self._on_dashboard_dpi_mux_toggled)
        self.dashboard_page.dpi_noise_toggled.connect(self._on_dashboard_dpi_noise_toggled)
        ...
        self.controller.zapret.error.connect(self.dashboard_page.set_zapret_error)
        ...
        self.config_fetch_page.fetch_now_requested.connect(self.controller.start_config_fetch)
        self.config_fetch_page.stop_fetch_requested.connect(self.controller.stop_config_fetch)
        self.config_fetch_page.settings_changed.connect(self._on_config_fetch_settings_changed)
        self.controller.config_fetch_started.connect(self.config_fetch_page.on_fetch_started)
        self.controller.config_fetch_progress.connect(self.config_fetch_page.on_fetch_progress)
        self.controller.config_fetch_finished.connect(self.config_fetch_page.on_fetch_finished)
    ...
    def _on_settings_changed(self, settings: AppSettings) -> None:
        ...
        self.config_fetch_page.set_settings(
            enabled=settings.config_fetch_enabled,
            interval_min=settings.config_fetch_interval_min,
            workers=settings.config_fetch_workers,
            filter_on=settings.config_fetch_filter,
            dedup=settings.config_fetch_dedup,
            extra_urls=list(settings.config_fetch_extra_urls or []),
        )

    def _on_config_fetch_settings_changed(self, d: dict) -> None:
        s = self.controller.state.settings
        s.config_fetch_enabled = d.get("enabled", s.config_fetch_enabled)
        s.config_fetch_interval_min = d.get("interval_min", s.config_fetch_interval_min)
        s.config_fetch_workers = d.get("workers", s.config_fetch_workers)
        s.config_fetch_filter = d.get("filter", s.config_fetch_filter)
        s.config_fetch_dedup = d.get("dedup", s.config_fetch_dedup)
        s.config_fetch_extra_urls = d.get("extra_urls", s.config_fetch_extra_urls)
        self.controller.save()
        self.controller._schedule_config_fetch()
    ...
    def _on_dashboard_auto_switch_toggled(self, checked: bool) -> None:
        from copy import deepcopy
        settings = deepcopy(self.controller.state.settings)
        settings.auto_switch_enabled = checked
        self.controller.update_settings(settings)

    def _on_dashboard_dpi_fragment_toggled(self, checked: bool) -> None:
        from copy import deepcopy
        settings = deepcopy(self.controller.state.settings)
        settings.dpi_fragment_enabled = checked
        self.controller.update_settings(settings)
        if checked:
            InfoBar.success("Анти-DPI", "Фрагментация TLS включена — применится при следующем подключении",
                            position=InfoBarPosition.TOP_RIGHT, duration=3000, parent=self)

    def _on_dashboard_dpi_mux_toggled(self, checked: bool) -> None:
        from copy import deepcopy
        settings = deepcopy(self.controller.state.settings)
        settings.dpi_mux_enabled = checked
        self.controller.update_settings(settings)

    def _on_dashboard_dpi_noise_toggled(self, checked: bool) -> None:
        from copy import deepcopy
        settings = deepcopy(self.controller.state.settings)
        settings.dpi_noise_enabled = checked
        self.controller.update_settings(settings)
    ...
    def _init_zapret_page(self) -> None:
        ...
        # Синхронизируем карточку Zapret на дашборде
        preset_names = [p.name for p in infos]
        self.dashboard_page.set_zapret_presets(preset_names, saved)
    ...
    def _quit_app(self) -> None:
        self._quitting = True
        self._save_geometry()
        # Stop the single-instance listener timer
        _btf = getattr(self, "_btf_timer", None)
        if _btf is not None:
            _btf.stop()
        _btf_evt = getattr(self, "_btf_event", None)
        if _btf_evt:
            import ctypes
            ctypes.windll.kernel32.CloseHandle(_btf_evt)  # type: ignore[attr-defined]
