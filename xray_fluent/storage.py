...
class StateStorage:
    ...
    def save(self, state: AppState) -> None:
        ...
        if not self._passphrase:
            raise PassphraseRequired(
                "Passphrase must be set before saving state — "
                "plain-text state.enc is not allowed on disk."
            )
    ...
    def export_backup(self, path: Path, passphrase: str = "") -> None:
        """Экспорт бэкапа. Без passphrase — plain JSON (только для dev/отладки).
        В prod-сборке main_window.py всегда запрашивает passphrase через UI."""
        state = self.load()
        payload = self._serialize_state(state)
        if passphrase:
            content = encrypt_with_passphrase(payload.encode("utf-8"), passphrase)
        else:
            # plain JSON backup — допустимо при явном намерении пользователя
            content = payload
